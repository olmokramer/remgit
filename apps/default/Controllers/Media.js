	...
	window.Media = {
			...
			$.ajax({
				...
				success: function(data){
					...
					$("#imagePicker").find('.mediaItem').unbind('click').click(function() {
						var imgCleanUrl = $(this).data('cleanurl');
		...
		initMenu: function(data) {
			...
			$("input#addImages").fileprocessor({
				...
				onItem: function(item, numProcessed, total) {
					...
					$.post("AjaxListener.php", data, function() {
						Media.uploadedFiles++;
		...
		*/
		...
		initList: function(data) {
			...
			$('.showmore').unbind("click").click(function(){
				$(this).remove();
				var options = "limit="+$(this).data('limitstart')+","+$(this).data('limitend');
		...
		*/
		...
		saveSingleVideo: function() {
			var url = $(".modalWindow input#url").val();
