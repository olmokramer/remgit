...
class Template {
	...
	private function selectTemplateFieldsByTemplateId($id) {
		...
		$sth = $pdo->prepare("SELECT fields.id, fields.label, fields.kind, fields.fieldtype, templates_fields.position FROM fields LEFT JOIN templates_fields ON fields.id = templates_fields.fields_id WHERE templates_fields.templates_id = :id ORDER BY templates_fields.position ASC");
		$sth->bindParam(":id", $id);
		$sth->execute();
